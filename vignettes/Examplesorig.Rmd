...
```{r, echo=TRUE}
    ...
    val$fold<-sample(1:10,length(val$Anon.Student.Id),replace=T)
...
#Load Assistments 2009 skillbuilder (example how to load a remote dataset)
```{r, echo=TRUE}
library(dplyr)
...
#val2<-val2[val2$attempt_count==1,]
val2<-val2[val2$skill!="",]

#Duplicate screening
val2 <-
     val2 %>%
     distinct(user_id,start_time,.keep_all=T)
...
datafile<-"C:/Users/ppavl/Dropbox/Active projects/algebra_2005_2006_train.txt" # CHANGE THIS VALUE TO THE DataShop export file IN YOUR R WORKING DIRECTORY
val2<-read.delim(colClasses = c("Anon.Student.Id"="character"),datafile,sep="\t", header=TRUE,quote="")
val2=as.data.table(val2)
#val2=as.data.table(val2)
val2$user_id<-val2$Anon.Student.Id


#val2<-val2[val2$attempt_count==1,]
val2<-val2[val2$KC.Default.!="",]

#Duplicate screening
val2 <-
     val2 %>%
     distinct(user_id,Step.Start.Time,.keep_all=T)
