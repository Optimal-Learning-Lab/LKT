...
for (i in 1:5) {
  ...
  modelob<-  buildLKTModel(
    usefolds = (1:5)[-i],data = val2, interc=TRUE,verbose=F,
    ...
    removefeat = c("intercept","logsuc$", "logfail$"),
      ...
      forv=5000,bacv=5000,allfeatures = c("intercept","lineafm","logafm","logsuc","logfail",
...
# Display the plot
ggsave(filename = paste("C:\\Users\\ppavl\\Dropbox\\Active projects\\",i,".png",sep=""),
       plot = gg,
       width = 6.3, height = 3.5, dpi = 600)
}
```
...
for (i in 1:5) {
  ...
  modelob<-  buildLKTModel(
    usefolds = (1:5)[-i],data = val3, interc=TRUE,
    removecomp = c("Anon.Student.Id","Anon.Student.Id","Anon.Student.Id"),
    removefeat = c("intercept","logsuc$", "logfail$"),
...
points(LASSO_Model$n_features[target_idx],LASSO_Model$auc_lambda[target_idx],col="red",pch=15)
plot(LASSO_Model$n_features,LASSO_Model$rmse_lambda,pch=16)
points(LASSO_Model$n_features[target_idx],LASSO_Model$rmse_lambda[target_idx],col="red",pch=15)
plot(LASSO_Model$n_features,LASSO_Model$BIC_lambda,pch=16)
points(LASSO_Model$n_features[target_idx],LASSO_Model$BIC_lambda[target_idx],col="red",pch=15)
...
kmeans_result_not_i <- kmeans(mean_data_not_i$mean_correct, centers = 9)
...
val3 <- val3 %>%
  mutate(
    difficuty_level = if_else(is.na(difficuty_level), "e", as.character(difficuty_level))
  )

val3[, difficuty_level := ifelse(.N <= 4, "e", difficuty_level), by = problem_id]
