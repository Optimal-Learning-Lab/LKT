...
#> Time difference of 2.720144 secs
#> Time difference of 31.90166 secs


str(modelob)
#> List of 6
#>  $ dropped 1se          : chr [1:15] "(Intercept)" "recency0.9KC..Default." "recency0.8KC..Default." "recency0.7KC..Default." ...
#>  $ retained 1se         : chr [1:10] "recency0.4KC..Default." "recency0.3KC..Default." "recency0.1KC..Default." "linefailKC..Default." ...
...
#> IRT  R2cv =   0.159412 R2 =   0.17526 
#> Faculty  R2cv =   0.1772982 R2 =   0.193067
...
#> 1          (Intercept) -0.2719469 -0.2408559         FALSE     0.95
#> 2 linefailKC..Default.  0.1691824  0.2580667         FALSE     0.95
#> 3  linesucKC..Default.  0.5413421  0.6424459         FALSE     0.95
#> 4    logitKC..Default.  0.3357042  0.5114883         FALSE     0.95
#> 5 logitAnon.Student.Id  0.6211200  0.6500147         FALSE     0.95
```
...
#> 
#> Step 1 results - pars  3  current BIC 62964.75 current AIC 62938 current AUC 0.7227457 current RMSE 0.4426974  CV McFadden's R2 0.171464 
#> recencysuc 
#>  KC..Default. 
#> pars 0.1515394 
#> 
...
#> 
#> Step 2 results - pars  4  current BIC 56803.36 current AIC 56767.69 current AUC 0.8218488 current RMSE 0.4138223  CV McFadden's R2 0.252055 
#> recencysuc numer 
...
#> 
#> Step 3 results - pars  6  current BIC 56429.86 current AIC 56376.36 current AUC 0.8276726 current RMSE 0.4107229  CV McFadden's R2 0.2545584 
...
#> 
#> Step 4 results - pars  8  current BIC 54549.18 current AIC 54477.84 current AUC 0.8392508 current RMSE 0.4032447  CV McFadden's R2 0.2831544 
...
#> 
#> Step 5 results - pars  9  current BIC 54447.46 current AIC 54367.21 current AUC 0.8396868 current RMSE 0.4027136  CV McFadden's R2 0.2825356 
...
#> 
#> Step 6 results - pars  9  current BIC 54447.46 current AIC 54367.21 current AUC 0.8396868 current RMSE 0.4027136  CV McFadden's R2 0.2825356 
#> recencysuc numer recencyfail propdec lineafm 
#>  KC..Default. CF..End.Latency. KC..Default. Anon.Student.Id Anon.Student.Id 
#> pars 1e-05 0.331292 0.99999
```
...
#> 
#> Step 0 results - pars  676  current BIC 61563.15 current AIC 55535.05 current AUC 0.8420717 current RMSE 0.4013434  CV McFadden's R2 0.265528 
...
#> 
#> Step 1 results - pars  643  current BIC 56251.39 current AIC 50517.57 current AUC 0.8713136 current RMSE 0.3807316  CV McFadden's R2 0.3321972 
...
#> lineafm$ KC..Cluster. 
#> 
#> Step 2 results - pars  607  current BIC 56275.02 current AIC 50862.22 current AUC 0.8687089 current RMSE 0.3827887  CV McFadden's R2 0.3277918 
...
#> 
#> Step 3 results - pars  543  current BIC 57389.82 current AIC 52547.72 current AUC 0.8573369 current RMSE 0.3910469  CV McFadden's R2 0.306431 
...
#> intercept Anon.Student.Id 
#> 
#> Step 4 results - pars  66  current BIC 59248.68 current AIC 58660.14 current AUC 0.8090632 current RMSE 0.4210114  CV McFadden's R2 0.2276412 
#> intercept recency 
...
#> 
#> Step 5 results - pars  68  current BIC 54020.86 current AIC 53414.49 current AUC 0.8461549 current RMSE 0.3988299  CV McFadden's R2 0.2966144 
...
#> 
#> Step 6 results - pars  68  current BIC 54020.86 current AIC 53414.49 current AUC 0.8461549 current RMSE 0.3988299  CV McFadden's R2 0.2966144 
#> intercept recency propdec 
#>  CF..Correct.Answer. KC..Default. Anon.Student.Id 
#> pars 0.3221993 0.9793741
```
...
#> 
#> Step 0 results - pars  849  current BIC 61001.05 current AIC 53430.26 current AUC 0.8564099 current RMSE 0.3915139  CV McFadden's R2 0.2948886 
...
#> intercept Anon.Student.Id 
#> 
#> Step 1 results - pars  372  current BIC 58531.57 current AIC 55214.33 current AUC 0.8387081 current RMSE 0.4035952  CV McFadden's R2 0.2736784 
...
#> intercept KC..Default. 
#> 
#> Step 2 results - pars  301  current BIC 57762.29 current AIC 55078.18 current AUC 0.8386732 current RMSE 0.4036166  CV McFadden's R2 0.273889 
...
#> logfail$ KC..Cluster. 
#> 
#> Step 3 results - pars  265  current BIC 57496.97 current AIC 55133.88 current AUC 0.8377984 current RMSE 0.4041899  CV McFadden's R2 0.2731946 
...
#> intercept KC..Cluster. 
#> 
#> Step 4 results - pars  230  current BIC 57266.58 current AIC 55215.6 current AUC 0.8367237 current RMSE 0.4049037  CV McFadden's R2 0.2719628 
...
#> logsuc$ KC..Cluster. 
#> 
#> Step 5 results - pars  194  current BIC 57235.32 current AIC 55505.36 current AUC 0.8344048 current RMSE 0.4064067  CV McFadden's R2 0.2682482 
#> logsuc logfail intercept logsuc$ logfail$ 
...
#> 
#> Step 6 results - pars  130  current BIC 58281.94 current AIC 57122.69 current AUC 0.8211743 current RMSE 0.4142556  CV McFadden's R2 0.2472414 
#> logsuc logfail intercept logsuc$ 
...
#> 
#> Step 7 results - pars  68  current BIC 54184.08 current AIC 53577.7 current AUC 0.8451487 current RMSE 0.3994975  CV McFadden's R2 0.294385 
...
#> logfail Anon.Student.Id 
#> 
#> Step 8 results - pars  67  current BIC 56996.33 current AIC 56398.87 current AUC 0.8259058 current RMSE 0.4114482  CV McFadden's R2 0.2573526 
...
#> logsuc Anon.Student.Id 
#> 
#> Step 9 results - pars  66  current BIC 59248.68 current AIC 58660.14 current AUC 0.8090632 current RMSE 0.4210114  CV McFadden's R2 0.2276412 
#> intercept recency 
#>  CF..Correct.Answer. KC..Default. 
#> pars 0.3158826 
#> 
...
#> propdec-Anon.Student.Id 66 59248.61 
#> 
#> Step 10 results - pars  68  current BIC 54020.86 current AIC 53414.49 current AUC 0.8461549 current RMSE 0.3988299  CV McFadden's R2 0.2966144 
#> intercept recency propdec 
#>  CF..Correct.Answer. KC..Default. Anon.Student.Id 
#> pars 0.3221993 0.9793741 
#> 
...
#> propdec-Anon.Student.Id 66 59248.57 
#> 
#> Step 11 results - pars  68  current BIC 54020.86 current AIC 53414.49 current AUC 0.8461549 current RMSE 0.3988299  CV McFadden's R2 0.2966144 
#> intercept recency propdec 
#>  CF..Correct.Answer. KC..Default. Anon.Student.Id 
#> pars 0.3221993 0.9793741
```
...
#> logsuc$ CF..Correct.Answer. 
#> 
#> Step 1 results - pars  65  current BIC 63428.69 current AIC 62849.07 current AUC 0.7463069 current RMSE 0.439556  CV McFadden's R2 0.1722426 
#> logsuc$ 
#>  CF..Correct.Answer. 
#> 
#> Step  2 start
...
#> 
#> Step 2 results - pars  65  current BIC 63428.69 current AIC 62849.07 current AUC 0.7463069 current RMSE 0.439556  CV McFadden's R2 0.1722426 
#> logsuc$ 
#>  CF..Correct.Answer.
```
